...
const Breakdown = () => {
  ...
  const [sortOption, setSortOption] = useState("allData") // Default sort option
  ...
  const validateBreakdownData = (breakdown) => {
    return breakdown && typeof breakdown === "object" && breakdown.key && breakdown.rakeNo && breakdown.reason
  }
  ...
  const fetchRakeDetails = useCallback(
            ...
            const plantRakeDetails = plant.rakeDetails
              ...
              .map((item) => {
                try {
                  ...
                  debug.totalBreakdownsFound += wagonShiftingDetails.length
    ...
    [lastFetchTime, appliedFilters.plant, getAuthConfig, parseSafeDate, normalizeString],
  )
    ...
    (breakdown) => {
      try {
        ...
        if (searchQuery) {
          ...
          const matchesSearch =
            (breakdown.rakeNo?.toString()?.toLowerCase()?.includes(lowercasedQuery) ||
              breakdown.key?.toString()?.toLowerCase()?.includes(lowercasedQuery) ||
              breakdown.reason?.toString()?.toLowerCase()?.includes(lowercasedQuery) ||
  ...
  // Get all breakdowns from all rakes
  const allBreakdowns = rakeDetails.flatMap((rake) => rake.wagonShiftingDetails || [])

  // Filter breakdowns based on search and date criteria
  const filteredBreakdowns = allBreakdowns.filter(safeFilterBreakdowns)
  ...
  //       "Rake No": breakdown.rakeNo || "N/A",
  //       "Wagon No": breakdown.key || "N/A",
  //       Reason: breakdown.reason || "N/A",
  //       "Break Down Time": breakdown.breakdownTime || "N/A",
  //     }))

  //     const worksheet = XLSX.utils.json_to_sheet(exportData)
  //     XLSX.utils.book_append_sheet(workbook, worksheet, "Breakdown Data")
  //     XLSX.writeFile(workbook, `breakdown_report_${new Date().toISOString().split("T")[0]}.xlsx`)
  //   } catch (error) {
  ...
  //     // Create worksheet with the exact structure
  //     const worksheet = XLSX.utils.aoa_to_sheet([
  //       ["BREAKDOWN REPORT"], // Title row
  //       ["Plant", "Rake No", "Wagon No", "Reasons", "Breakdown Time"], // Header row
  //     ]);

  //     // Add data rows
  //     const dataRows = filteredBreakdowns.map(breakdown => [
  //       breakdown.plant || "",
  //       breakdown.rakeNo || "",
  //       breakdown.key || "",
  //       breakdown.reason || "",
  ...
  //     // Set exact column widths as in the image
  //     worksheet['!cols'] = [
  //       { wch: 15 }, // Plant
  //       { wch: 15 }, // Rake No
  //       { wch: 15 }, // Wagon No
  //       { wch: 50 }, // Reasons
  //       { wch: 20 }, // Breakdown Time
  //     ];

  //     // Add the worksheet to the workbook
  //     XLSX.utils.book_append_sheet(workbook, worksheet, "Breakdown Data");

  //     // Download only the Excel file without generating the HTML page
  //     XLSX.writeFile(workbook, `breakdown_report_${new Date().toISOString().split("T")[0]}.xlsx`);
  ...
  //     // Apply formatting to the title row - center align and larger font
  //     const titleStyle = {
  //       alignment: { horizontal: "center", vertical: "center" },
  //       font: { bold: true, sz: 16 } // Increased font size to 16
  //     };

  //     // Apply formatting to header row - center align and bold
  //     const headerStyle = {
  //       alignment: { horizontal: "center", vertical: "center" },
  //       font: { bold: true }
  //     };

  //     // Apply styles to title row (A1:E1)
  //     ["A1", "B1", "C1", "D1", "E1"].forEach(cell => {
  //       if (!worksheet[cell]) worksheet[cell] = {};
  //       worksheet[cell].s = titleStyle;
  //     });

  //     // Apply styles to header row (A2:E2)
  ...
  const downloadReport = useCallback(() => {
    try {
      ...
      // Add cell styles using the proper format for XLSX
      // Each cell needs proper formatting
      for (let i = 0; i < 5; i++) {
          ...
          {loading ? (
            ...
            <tbody>
              <tr>
                <td colSpan="5" className="no-data-message">
                  {error ? "Error loading data" : "No breakdown data found"}
                </td>
              </tr>
            </tbody>
          ) : (
            <tbody>
              {filteredBreakdowns.map((breakdown, index) => (
                <tr key={`${breakdown.rakeNo}-${breakdown.key}-${index}`}>
                  <td>{breakdown.plant || "N/A"}</td>
                  <td>{breakdown.rakeNo || "N/A"}</td>
                  <td>{breakdown.key || "N/A"}</td>
                  <td>{breakdown.reason || "N/A"}</td>
                  <td>{breakdown.breakdownTime || "N/A"}</td>
                </tr>
              ))}
            </tbody>
          )}
