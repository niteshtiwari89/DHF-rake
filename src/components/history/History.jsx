...
const History = () => {
                ...
                plantsData.forEach((plant) => {
                    ...
                    if (plant.rakeDetails && Array.isArray(plant.rakeDetails)) {
                        ...
                        const plantRakeDetails = plant.rakeDetails
                            ...
                            // Filter to only include checked out entries
    ...
    const safeFilter = useCallback(
        (rake) => {
            try {
                // First, ensure this is a checked out entry (already filtered in fetchRakeDetails but double-check)
                if (!rake.isCheckedOut) return false;
    ...
    return (
        <div className="home-container">
            <div className="dashboard-header">
                <h2>Rake Entry Dashboard - History</h2>
                        ...
                        <tbody>
                            <tr>
                                <td colSpan="10" className="no-data-message">
                                    {error ? "Error loading data" : "No checked out rakes found for the selected filters"}
                                </td>
                            </tr>
                        </tbody>
                    ) : (
...
const calculateTiming = (checkInTime, checkOutTime) => {
