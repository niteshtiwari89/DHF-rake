    ...
    const downloadReport = useCallback(() => {
        try {
            ...
            const worksheet = XLSX.utils.aoa_to_sheet([
                ["UNRESOLVED BREAKDOWNS REPORT"], // Title row
            ...
            // Convert JSON to sheet starting from row 3 (after title and empty row)
            const dataSheet = XLSX.utils.json_to_sheet(exportData);

            // Copy the data from json_to_sheet to our worksheet with the title
            ...
            for (let i = 0; i < headers.length; i++) {
                ...
                if (!worksheet[titleCellRef]) worksheet[titleCellRef] = { v: "" };
                ...
                const headerCellRef = XLSX.utils.encode_cell({ r: 2, c: i });
            ...
            const fileName = `unresolved_breakdowns_report_${new Date().toISOString().split("T")[0]}.xlsx`;
    ...
    return (
        <div className="unresolved-breakdown-container">
            <div className="dashboard-header">
                <h2>Resolved Breakdown History Dashboard</h2>
